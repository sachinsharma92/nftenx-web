...
const TitlesDescriptionButtonHero = (props: PropTypes) => {
  return <article className="relative p-section lg:py-16 lg:-px-section flex justify-center items-center text-secondary-1 join-waitlist">
    ...
    <div className="lg:w-2/3 flex flex-col gap-2 text-center justify-center items-center">
      ...
      {props.subtitle && <H1 className='text-subtitle'>{props.subtitle}</H1>}
      {props.description && <p className='text-primary-light font-mono'>{props.description}</p>}
      {props.linkTitle && <A passHref href={props.linkHref ?? 'javascript:void(0)'} className="mt-6 text-secondary-1 py-4"><a className='text-secondary-1'>{props.linkTitle}</a></A>}
    </div>
  </article>
}
